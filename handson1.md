...
## 2. Red Hat OpenShift Pipelinesのインストール
...
OpenShiftのWebコンソールへ戻り、[プロジェクト]ボタンをクリックします。([ホーム]セクションを閉じている場合は、そちらを開いてください)
...
言語やタイプの一覧がタイルで表示され、.NETが選択されていることを確認します。今回GitHubリポジトリーへForkしたプロジェクトは .Net アプリケーションだからです。
...
### 3.7 自動生成されたパイプラインの確認

自動生成されたパイプラインを確認していきます。

[詳細],[リソース],[モニタリング]とタブが３つありますが、[リソース]タブを選択してください。
