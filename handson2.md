...
ログの最下段を見ると1件Failedとなっています。

原因を探るためにログを上にスクロールし中盤あたりに移動すると、
```Assert.Contains() Failure```, ```Not found: Success```と記載があります。

このテストでは、「Success」という文字列がWebサイトに含まれていることを期待したが、含まれていなかったことを示唆しています。

> もし上記文字列をログ内に見つけられない方は、ブラウザの内部キーワード検索を利用いただき、「Assert.Contains() Failure」を検索ください。
...
> 「2.4 パイプラインの動作確認(テスト失敗)」では手動でテストを実行しましたが、今回はソースコードを変更したことによりGitHubとトリガが連動しパイプラインでテストが自動実行されています。
...
> 「パイプライン実行>パイプライン実行 の詳細」ページには複数タブがあります。
>
> [詳細]タブが選択されている場合は、[ログ]タブを選択してください。

前回は**1件Failed**となっていましたが、今回は**1件Passed**、つまりテスト成功となっています。
